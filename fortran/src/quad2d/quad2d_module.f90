module quad2dModule
  ...
  type, public :: tLayerModel
    ...
    !
    integer(I4B) :: nlay_ref
    integer(I4B), dimension(:,:), pointer :: lay_ref => null()
  contains
    procedure :: init            => tLayerModel_init
    procedure :: clean           => tLayerModel_clean
    procedure :: read_extent     => tLayerModel_read_extent
    procedure :: read_extent_ref => tLayerModel_read_extent_ref
    procedure :: read_xy         => tLayerModel_read_xy
    procedure :: read_xy_ref     => tLayerModel_read_xy_ref
    procedure :: get_nlay        => tLayerModel_get_nlay
  ...
  !
  integer(I4B), parameter :: i_const     = 4
  !
  integer(I4B), parameter :: i_vrt_array_chd = 5
  ...
  contains
    ...
    procedure :: mf6_get_data_vrt_array_array_chd => tDataModelData_mf6_get_data_vrt_array_array_chd
  ...
  contains
    ...
    procedure :: get_dm_ptr     => tDataModels_get_dm_ptr
    procedure :: get_dm_idx     => tDataModels_get_dm_idx
  ...
  integer(I4B), parameter, public :: i_dat_mod            = 5
  integer(I4B), parameter, public :: i_tgt_cs_min         = 6
  integer(I4B), parameter, public :: i_tgt_cs_min_lay_beg = 7
  integer(I4B), parameter, public :: i_tgt_cs_min_lay_end = 8
...
! ------------------------------------------------------------------------------
    ...
    integer(I4B) :: ig, i, nod, bs, ig_max_I1, il_max_I2, ig_max, n
    real(R8B) :: xc, yc
...
! ------------------------------------------------------------------------------
    ...
    do il = 1, nl; do ir = 1, nr; do ic = 1, nc
      ...
      if (nod > 0) then
        ...
        ig = ngrid - int(nint(log(real(bb%ncol, R8B))/log(2.d0)), I4B) !nint fix
      ...
      do ir = 1, nr; do ic = 1, nc
        ...
        if (nod > 0) then
          if (flg(nod) == 0) then
            ...
            call icrl_to_node(n, jc, jr, 1, g%nc, g%nr)
            if (x(ic,ir,il) > 0) then
              this%map_nod(nod) = n
            else
              this%map_nod(nod) = -n
            end if
...
! ------------------------------------------------------------------------------
    ...
    do ir = 1, nr; do ic = 1, nc
      do il = 1, nl
        n = abs(this%grid_x_nod(ic,ir,il))
        if (n > 0) then
          top_nodes(n) = 1
          exit
        end if
  ...
  subroutine tMF6Disu_x_to_top_nodes_grid(this, top_nodes_grid, sat)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    real(R8B), dimension(:), intent(in) :: sat
    ! -- local
    integer(I4B) :: il, ir, ic, nl, nr, nc, n
! ------------------------------------------------------------------------------
    if (.not.allocated(this%grid_x_nod)) then
...
! ------------------------------------------------------------------------------
    ...
    do i = 1, this%nodes
      ...
      n = abs(this%map_nod(i)); top = this%top(i); bot = this%bot(i)
    ...
    !
    ! set the external boundary
    do il = 1, this%nlay_act
      mg => mga_nod%get_mg(il)
      do ig = 1, this%ngrid
        g => mg%grid(ig)
        call g%set_int_mv_from_xi4()
      end do
    end do
...
! ------------------------------------------------------------------------------
    ...
    !
    ! realised ngrid:
    f = log(this%cs_max_rea/this%cs_min_rea)/log(2.d0)
    this%ngrid = int(nint(f),I4B) + 1
...
! ==============================================================================
! ==============================================================================
   subroutine tDataModels_init(this, f_csv)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    type(tCSV) :: csv
    type(tdataModel), pointer :: dat_mod => null()
    character(len=MXSLEN) :: id, f_list, f_map
    integer(I4B) :: ir, ic1, ic2, ic3
! ------------------------------------------------------------------------------
    ...
    allocate(this%dat_mods(this%n_inp_mod))
    ic1 = csv%get_col('id')
    ic2 = csv%get_col('f_parameters_csv')
    ic3 = csv%get_col('f_mapping_csv')
    
    do ir = 1, this%n_inp_mod
       dat_mod => this%dat_mods(ir)
       call csv%get_val(ir=ir, ic=ic1, cv=id)
       call csv%get_val(ir=ir, ic=ic2, cv=f_list)
       call csv%get_val(ir=ir, ic=ic3, cv=f_map)
       call dat_mod%init(id, f_list, f_map)
    end do
    allocate(this%uni_map_id(NMAX_UNI_MAP))
    allocate(this%uni_map_ir(this%n_inp_mod,NMAX_UNI_MAP))
    this%n_uni_map = 0
    this%uni_map_id = ''
    this%uni_map_ir = 0
    !
...
! ------------------------------------------------------------------------------
  ...
  function tDataModels_get_dm_ptr(this, cid) result(dm_ptr)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    type(tdataModel), pointer :: dm_ptr
    ! -- local
    integer(I4B) :: i, j, n
! ------------------------------------------------------------------------------
    j = -1
    do i = 1, this%n_inp_mod
      if (this%dat_mods(i)%name == cid) then
        j = i; exit
      end if
    end do
    
    if (j < 0) then
      call errmsg('tLayerModels_get_dm_ptr: id '//trim(cid)//' not found.')
    else
      dm_ptr => this%dat_mods(j)
    end if

    return
  end function tDataModels_get_dm_ptr
  
  function tDataModels_get_dm_idx(this, cid) result(dm_idx)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------
    ...
    return
  end function tDataModels_get_dm_idx
...
! ------------------------------------------------------------------------------
    ...
    if (associated(this%vrt)) then
      call this%vrt%clean()
      deallocate(this%vrt); this%vrt => null()
    end if
    if (associated(this%vrta)) then
      call this%vrta%clean()
      deallocate(this%vrta); this%vrta => null()
    end if
    if (associated(this%csv)) then
      call this%csv%clean()
      deallocate(this%csv); this%csv => null()
    end if
    
   return
  end subroutine tData_clean
...
! ------------------------------------------------------------------------------
    ...
    character(len=MXSLEN) :: s, s1, s2, s3, file, file_type, assign_method, sconst
    !
...
! ------------------------------------------------------------------------------
    ...
    do i = 1, dmdat%ndat
      ...
      call parl%get_val(ir=ir, ic=parl%get_col('file_type'), cv=file_type)
      if (len_trim(file_type) == 0) then
        file_type = 'constant'
        dat%lconst = .true.
      else
        dat%lconst = .false.
      ...
      case('vrta')
        if (dmdat%i_type == i_type_list) then
          dmdat%i_in_file_type = i_vrt_array_chd
        else
      ...
      case('constant')
        dmdat%i_in_file_type = i_vrt
        dat%i_type = i_const
        if (.not.parl%exist_col('constant')) then
          call errmsg('Field constant not found.')
        end if
      ...
      ! addition value
      dat%add_val = R8ZERO
      if (parl%exist_col('add_val')) then
        call parl%get_val(ir=ir, ic=parl%get_col('add_val'), cv=s)
        if (len_trim(s) > 0) then
          read(s,*) dat%add_val
        end if
      end if
    ...
    do i = 1, dmdat%ndat
      dat => dmdat%dat(i)
      if (dat%i_type == i_const) then
        i_min = min(i_min, i_vrt)
        i_max = max(i_max, i_vrt)
      else
...
! ------------------------------------------------------------------------------
  ...
  function tLayerModels_get_lm_ptr(this, cid) result(lm_ptr)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    class(tLayerModels) :: this
    character(len=*), intent(in) :: cid
    type(tLayerModel), pointer :: lm_ptr
...
! ------------------------------------------------------------------------------
    j = -1
    do i = 1, this%n_inp_mod
      if (this%lay_mods(i)%name == cid) then
        j = i; exit
      end if
    end do
    
    if (j < 0) then
      call errmsg('tLayerModels_get_lm_ptr: id '//trim(cid)//' not found.')
    else
      lm_ptr => this%lay_mods(j)
    end if
      
    return
  end function tLayerModels_get_lm_ptr
...
! ==============================================================================
   ...
   subroutine tLayerModel_init(this, idx, name, nlay, f, f_type, f_kh, f_ref)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    character(len=*), intent(in), optional :: f_ref
    !
    ! -- local
    type(tVrt),      pointer :: vrt => null()
    type(tVrtArray), pointer :: vrta => null()
    ...
    !
    integer(I4B) :: il, ir
    integer(I4B), allocatable, dimension(:) :: ilay_ref, nlay_ref
! ------------------------------------------------------------------------------
    ...
    !
    if (this%nlay == 0) then
      call errmsg('tLayerModel_init: nlay = 0.')
    end if
    !
    ...
    !
    if (present(f_ref)) then
      allocate(csv)
      call csv%read(f_ref)
      call csv%get_column(key='layer', i4a=ilay_ref)
      call csv%get_column(key='nlay',  i4a=nlay_ref)
      this%nlay_ref = size(ilay_ref)
      allocate(this%lay_ref(2, this%nlay_ref))
      do ir = 1, this%nlay_ref
        this%lay_ref(1, ir) = ilay_ref(ir)
        this%lay_ref(2, ir) = nlay_ref(ir)
      end do
      deallocate(ilay_ref, nlay_ref)
    else
      this%nlay_ref = 0
      this%lay_ref => null()
    end if
...
! ------------------------------------------------------------------------------
    ...
    !
    this%nlay_ref = 0
    if (associated(this%lay_ref)) then
      deallocate(this%lay_ref)
    end if
    !
    return
...
!  end subroutine tLayerModel_read_full_grid
  ...
  function tLayerModel_get_nlay(this) result(nlay)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------
    nlay = this%nlay
    !
    if (this%nlay_ref > 0) then
      n = 0
      do i = 1, this%nlay_ref
        n = n + this%lay_ref(2,i) - 1
      end do
      nlay = nlay + n
    end if
    !
    return
  end function tLayerModel_get_nlay
   
  subroutine tLayerModel_read_xy_ref(this, x, y, cs, zp, khp, mv, lay_act, nl_act)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    real(R4B), dimension(:,:), allocatable :: zp_all_wrk, zp_all, khp_all
    real(R4B), dimension(:), allocatable :: r4wk
    real(R4B) :: mvr4_read, top, bot, thk, r4const, frac
    integer(I4B) :: ip, np, i_uscl, i_dscl, il, jl, kl, i, nl, nlay, nlay_extra
! ------------------------------------------------------------------------------
    ...
    if ((np /= size(y)).or.(np /= size(cs))) then
      call errmsg('tLayerModel_read_xy_ref: invalid dimensions.')
    end if
    !
    ...
    !
    if (this%compressed) then
      call errmsg('tLayerModel_read_xy_ref: compressed not yet supported.')
    else
      allocate(zp_all_wrk(np,this%nlay+1))
      !
      ...
      do il = 1, this%nlay + 1
        ...
        do ip = 1, np
          if (zp_all_wrk(ip,il) == mvr4_read) then
            zp_all_wrk(ip,il) = mv
          end if
        end do
      end do
      !
      allocate(i4wk(this%nlay)); i4wk = 1
      do i = 1, this%nlay_ref
        il = this%lay_ref(1,i)
        nl = this%lay_ref(2,i)
        i4wk(il) = nl
      ...
      nlay = sum(i4wk)
      !
      allocate(zp_all(np,nlay+1))
      if (allocated(lay_act)) deallocate(lay_act)
      allocate(lay_act(nlay)); lay_act = 0
      !
      do ip = 1, np
        kl = 0
        do il = 1, this%nlay
          top = zp_all_wrk(ip,il)
          bot = zp_all_wrk(ip,il+1)
          ...
          do jl = 1, nl-1
            kl = kl + 1
            frac = real(jl,R4B)/real(nl,R4B)
            if (thk /= mv) then
              zp_all(ip,kl) = top - frac*thk
              if (abs(thk) > R4TINY) then
                lay_act(kl) = 1
              end if
            else
      ...
      !
      ! cleanup
      if (allocated(zp_all_wrk)) deallocate(zp_all_wrk)
      if (allocated(i4wk))       deallocate(i4wk)
      !
      if (associated(this%kh_vrta)) then
        lkhp = .true.
        kh_vrta => this%kh_vrta
        allocate(khp_all(np, nlay))
      else
        lkhp = .false.
      end if
      !
      if (lkhp) then
        do il = 1, nlay
          ...
          if (lconst) then
            r4wk = r4const * kh_vrta%mult_fac(il) + kh_vrta%add_val(il)
          else
            vrt => kh_vrta%get_vrt(il)
            call vrt%read_xy(x=x, y=y, cs=cs, r4a=r4wk, mvr4=mvr4_read, &
              i_uscl=i_uscl_geom, i_dscl=i_dscl_nointp, &
              mfr8=kh_vrta%mult_fac(il), avr8=kh_vrta%add_val(il))
          end if
          khp_all(:,il) = r4wk
          !
          ! replace missing value
          do ip = 1, np
            if (khp_all(ip,il) == mvr4_read) then
              khp_all(ip,il) = mv
            end if
          end do
        end do ! il = 1, nlay
    ...
    !
    return
  end subroutine tLayerModel_read_xy_ref
    
  subroutine tLayerModel_read_extent_ref(this, bbx, zp, mv, lay_act, nl_act, mask)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    real(R4B) :: mvr4_read, thk, top, bot, r4const, frac
    real(R4B), dimension(:,:), allocatable :: r4wk, top2d, bot2d, thk2d
    real(R4B), dimension(:,:,:), allocatable :: zp_read, zp_read_wk
    real(R8B) :: x, y
    integer(I4B) :: il, jl, kl, i_uscl, i_dscl, nc, nr, mc, mr, ir, ic
    integer(I4B) :: ntile, nl, i, nlay
    integer(I4B), dimension(:), allocatable :: i4wk
! ------------------------------------------------------------------------------
    ! up- and downscaling parameters
    i_uscl = i_uscl_arith
    ...
    !
    if (this%compressed) then
      call errmsg('tLayerModel_read_extent_ref: compressed is not yet supported.')
    else
      allocate(zp_read_wk(nc,nr,this%nlay+1))
      vrta => this%laymod_vrta
      !
      do il = 1, this%nlay + 1
        if (allocated(r4wk)) deallocate(r4wk)
        ...
        if (lconst) then
          ...
          call vrt%read_extent(xr4=r4wk, mvr4=mvr4_read, bbx=bbx, &
            i_uscl=i_uscl_arith, i_dscl=i_dscl_nointp, &
            mfr8=vrta%mult_fac(il), avr8=vrta%add_val(il))
          call vrt%clean_x()
        end if
        !
        mc = size(r4wk,1); mr = size(r4wk,2)
        if ((mc /= nc).or.(mr /= nr)) then
          call errmsg('tLayerModel_read_extent_ref: inconsistent dimensions.')
        end if
        !
        if (il == 1) then
          mv = mvr4_read
      ...
      nlay = sum(i4wk)
      allocate(zp_read(nc,nr,nlay+1)); zp_read = mv
      allocate(top2d(nc,nr), bot2d(nc,nr), thk2d(nc,nr))
      kl = 0
      do il = 1, this%nlay
        top2d = zp_read_wk(:,:,il)
        bot2d = zp_read_wk(:,:,il+1)
        thk2d = mv
        do ir = 1, nr; do ic = 1, nc
          if ((top2d(ic,ir) /= mv).and.(bot2d(ic,ir) /= mv)) then
            thk2d(ic,ir) = top2d(ic,ir) - bot2d(ic,ir)
          end if
        end do; end do
        !
        nl = i4wk(il)
        ...
        do jl = 1, nl-1
          ...
          frac = real(jl,R4B)/real(nl,R4B)
          do ir = 1, nr; do ic = 1, nc
            if ((top2d(ic,ir) /= mv).and.(thk2d(ic,ir) /= mv)) then
              zp_read(ic,ir,kl) = top2d(ic,ir) - frac*thk2d(ic,ir)
            end if
          end do; end do
        end do
      end do
      ...
      do il = 1, nlay
        ...
        do ir = 1, nr
          do ic = 1, nc
            top = zp_read(ic,ir,il)
            bot = zp_read(ic,ir,il+1)
            if ((top /= mv).and.(bot /= mv)) then
              thk = top - bot
              if (abs(thk) > R4TINY) then
                nl_act = nl_act + 1
      ...
      !
      if (nl_act == 0) then
        call errmsg('tLayerModel_read_extent_ref: no layers found.')
      end if
      !
      ...
      !
      ! clean up
      if (allocated(zp_read_wk)) deallocate(zp_read_wk)
    ...
    do ir = 1, nr; do ic = 1, nc
      top = zp(ic,ir,1)
      do il = 2, nl_act + 1
        !if ((ir==57).and.(ic==152).and.(il==30)) then
        !  write(*,*) '@@@@'
        !end if
        bot = zp(ic,ir,il)
        if ((top /= mv).and.(bot /= mv).and.(mask(ic,ir) == 1)) then
          if (top < bot) then
            call get_xy(x, y, ic, ir, bbx%xll, bbx%yur, bbx%cs)
            write(*,'(a)') '('//ta((/x/))//','//ta((/y/))//'): top='//ta((/top/))//' bot='//ta((/bot/))
            write(*,'(a)') 'il='//ta((/il/))//'ir='//ta((/ir/))//'ic='//ta((/ic/))
            call errmsg('tLayerModel_read_extent_ref: inconsistent layer model')
          end if
        end if
        top = bot
      end do
    ...
    !
    return
  end subroutine tLayerModel_read_extent_ref
...
! ------------------------------------------------------------------------------
  ...
  subroutine tLayerModel_read_extent(this, bbx, zp, mv, lay_act, nl_act, mask)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------
    ...
    do lid = 1, this%n
      ...
      if (q%get_flag(active=LDUM)) then
        ...
        if (lay_mod%nlay_ref > 0) then
          call lay_mod%read_xy_ref(x, y, cs, zp, khp, mv, nl_act, nl)
        else
...
! ------------------------------------------------------------------------------
  ...
  subroutine tQuads_write_mf6_grid(this, lid0, lid1, &
    ...
    f_vrt_pref, write_nod_map, filter_sat, &
...
! ------------------------------------------------------------------------------
    ...
    logical, intent(in) :: filter_sat
    character(len=*), intent(in) :: vtk_lid
    character(len=*), intent(in) :: f_lay_mod_output_csv
    ...
    character(len=MXSLEN), dimension(:), allocatable :: f_tile_nodmap, mv_tile_nodmap, f_vtu, lab_vtu
    character(len=MXSLEN) :: f, d, f_csv_dat, vtk_csv, clm, s
    character(len=MXSLENLONG) :: s_long
    real(R8B), dimension(:,:), allocatable :: vtk_xc, vtk_x, sat
    ...
    integer(I4B) :: ic0, ir0, ic1, ir1, ic, ir, jc, jr, kc, kr, ilm, il, jl, ig, k
...
! ------------------------------------------------------------------------------
    ...
    call csv_out%read(f_lay_mod_output_csv)
    if (csv_out%get_nr() /= nmod) then
      call errmsg('Invalid output layer definitions [mf6_post]')
    end if
    !
    ...
    if (write_vtk) then
      ...
      if (s == 'all') then
        ...
        allocate(lid_arr_vtk(this%n_act)); k = 0
        do lid = lid0, lid1
          q => this%get_quad(lid)
          if (q%get_flag(active=LDUM)) then
            k = k + 1
      ...
      do ilid = 1, nlid_vtk
        ...
        if (q%get_flag(active=LDUM)) then
          ...
          if (csv_out%exist_col('vtk_csv')) then
            call q%get_prop_csv(ikey=i_lay_mod, cv=clm) ! layer model
            ir = csv_out%get_row(key_val=clm, hdr_key='id')
            call csv_out%get_val(ir=ir, ic=csv_out%get_col('vtk_csv'), cv=f)
          ...
          if (allocated(f_vtu)) deallocate(f_vtu)
        ...
        do lid = lid0, lid1
          ...
          if (q%get_flag(active=LDUM)) then
            ...
            if (bbx_intersect(bbx, bbxp)) then
              ...
              if (filter_sat) then
                if (merge) then
                  call errmsg('tQuads_write_mf6_grid: merge for saturation filtering not yet supported.')
                else
                  if (allocated(sat)) deallocate(sat)
                  call q%grid_init(read_iajaihc=.true.)
                  call q%get_prop_csv(ikey=i_budget, cv=f); call swap_slash(f)
                  allocate(post)
                  call post%init(f, kper, kper, q%disu%nodes)
                  call post%read_saturation(size(q%disu%ja), sat)
                  call post%clean()
                  deallocate(post)
                end if
              end if
              ...
              if (write_budget) then
                if (merge) then
                  ...
                  if (filter_sat) then
                    call q%get_nod_map(flf_layer, nodmap_q, nodes, clean_disu=.false., read_iajaihc=.true., sat=sat(1,:))
                  else
                ...
                if (merge) then
                  ...
                  if (filter_sat) then
                    call q%get_nod_map(il, nodmap_q, nodes, sat=sat(1,:))
                  else
  ...
  subroutine tQuads_write_mf6_point(this, f_in_point_csv, kper_beg, &
    kper_end, f_out_point_csv, filter_sat)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    type(tMF6Disu), pointer :: disu => null()
    !
    integer(I4B), dimension(:), allocatable :: layer_arr, lid_arr, x_active
    integer(I4B), dimension(:), allocatable :: xmap, hdr_i_type, kper_arr
    integer(I4B), dimension(:,:), allocatable :: nodes_read
    real(R8B), dimension(:), allocatable :: r8w
    real(R8B), dimension(:,:), allocatable :: x_arr, heads_read, heads_out, sat
    real(R8B), dimension(2,1) :: xy
    character(len=MXSLEN), dimension(:), allocatable :: hdr_keys
    !
    character(len=MXSLEN) :: f, s, id
    logical :: lflag, layer_found, lfound, llayer, lz, lnod, lpointinbb, lskip_point
    integer(I4B) :: nx, mx, lid, lid_read, ix, jx, ic, ir, il, jl, ilay, jlay, nod, n, i, nx_active
    integer(I4B) :: iper, kper, nper, il_min, il_max, il_top, il_bot, il_found
    real(R8B) :: z, top, bot
! ------------------------------------------------------------------------------
    ...
    do ix = 1, nx
      ...
      do iper = 1, nper
        s = 'nod_' // ta((/iper/), "(i0.6)")
        call csv_in%set_val_by_key(key=s, ir=ix, i4v=0)
      end do
      ...
      if (q%get_flag(active=LDUM)) then
        ...
        if (lflag) then
            ...
            if (lpointinbb) then
              ...
              do ix = 1, nx
                ...
                if (.not.layer_found) then
                  ...
                  call errmsg('Error: specified layer not found for id '//trim(id)//'!')
                end if
                !
                if (layer_found) then
                  nod = abs(disu%grid_x_nod(ic,ir,il_found))
                  ilay = disu%lay_act(il_found)
                  s = 'nod_' // ta((/nper/), "(i0.6)")
                  call csv_in%get_val(ir=ix, key=s, i4v=n)
                  if (n == 0) then
                    mx = mx + 1
                    x_active(ix) = 1
                    call csv_in%set_val_by_key(key='lid', ir=ix, i4v=lid)
                    call csv_in%set_val_by_key(key=s, ir=ix, i4v=nod)
          ...
          if (mx > 0) then
            ...
            allocate(xmap(mx), nodes_read(nper, mx), heads_read(nper, mx)); heads_read = R8MV
            mx = 0
            do ix = 1, nx
              ...
              if (lid_read == lid) then
                ...
                do iper = 1, nper
                  s = 'nod_' // ta((/iper/), "(i0.6)")
                  call csv_in%get_val(ir=ix, key=s, i4v=nod)
                  nodes_read(iper, mx) = nod
                end do
              end if
            end do
            !
...
! ------------------------------------------------------------------------------
    ...
    integer(I4B), dimension(:), allocatable :: nodes_flag
    integer(I4B), dimension(:,:), allocatable :: nodes_read
...
! ------------------------------------------------------------------------------
    ...
    do lid = 1, this%n
      ...
      if (q%get_flag(active=LDUM)) then
        ...
        if (n > 0) then
          if (allocated(nodes_read)) deallocate(nodes_read)
          allocate(nodes_read(1, n), heads_read(nper, n))
          n = 0
          do i = 1, disu%nodes
          ...
          do iper = 1, nper
            ...
            call wbd%write_list(id=id, i4a1=nodes_read(1,:), r8a1=heads_read(iper,:), f_bin=f_bin)
          end do
          call wbd%write_csv()
          !
        end if
    ...
    do ir = 1, nr
      do ic = 1, nc
        if (x(ic,ir) == x_src) then
          ...
          !
          if ((.not.labeled).and.((ic == 1).or.(ic == nc).or.(ir == 1).or.(ir == nr))) then
            labeled = .true.
            m = m + 1
            int_mv_icir_wk(1,m) = ic; int_mv_icir_wk(2,m) = ir
            ext_mv_icir_wk(1,m) = jc; ext_mv_icir_wk(2,m) = jr
...
! ------------------------------------------------------------------------------
    ...
    real(R8B), dimension(:), optional, intent(in) :: sat
    ! -- local
    type(tMF6Disu), pointer :: disu => null()
    type(tBB) :: bbi
    integer(I4B) :: jl, ir, ic, n, nc, nr
    logical :: lclean, liajaihc
    real(R8B), dimension(:), allocatable :: fsat
! ------------------------------------------------------------------------------
    ...
    allocate(fsat(nodes))
    if (present(sat)) then
      if (size(sat) /= nodes) then
        call errmsg('tQuad_get_nod_map: invalid dimension saturation vector.')
      end if
      fsat = sat
    else
      fsat = R8ONE
    ...
    !
    if (il == 0) then
      call disu%x_to_top_nodes_grid(nod_map, fsat)
    else
      do jl = 1, disu%nlay_act
...
! ------------------------------------------------------------------------------
    ...
    do ilev = ilev0, ilev1, ilevi
      ...
      !call get_bnd_i4_5p(g0%xi4, -1, 1, int_icir=icir0) ! 11-02-25
      call get_bnd_i4_5p(g0%xi4, -1, 1, int_icir=icir0_int, int_mv_icir=g0%icir_int_mv)
      n_int = 0; g0%n_int_mv = 0
      if (allocated(icir0_int)) n_int = size(icir0_int,2)
      ...
      if (n_tot == 0) then
        ...
        do i = 1, n_int
          n_tot = n_tot + 1
          icir0(1,n_tot) = icir0_int(1,i)
          icir0(2,n_tot) = icir0_int(2,i)
        end do
        do i = 1, g0%n_int_mv
          n_tot = n_tot + 1
          icir0(1,n_tot) = g0%icir_int_mv(1,i)
          icir0(2,n_tot) = g0%icir_int_mv(2,i)
        end do
      end if
    ...
    !
    ! set the external boundary in case of only 1 grid
    if (ngrid == 1) then
      g0 => xlev%x(1)
      call get_bnd_i4_5p(g0%xi4, -1, 1, int_icir=icir0_int, int_mv_icir=g0%icir_int_mv)
      if (allocated(g0%icir_int_mv)) then
        g0%n_int_mv = size(g0%icir_int_mv,2)
      end if
    end if
    ...
    do igrid = 1, ngrid
      ...
      do ilev = ilev0, ilev1, ilevi
        ...
        if (xlev%x(ilev)%n_int_mv > 0) then
          call mg%grid(jlev)%set_int_mv(xlev%x(ilev)%icir_int_mv)
        end if
    ...
    if (allocated(icir0)) deallocate(icir0)
    if (allocated(icir0_int)) deallocate(icir0_int)
...
! ------------------------------------------------------------------------------
    ...
    integer(I4B), dimension(:,:), allocatable :: xi4_int_mv
    integer(I4B) :: nc_f, nr_f, nc_ig, nr_ig, nc_jg, nr_jg, jg, kg, ic, ir, jc, jr, kc, kr
    integer(I4B) :: i, j, k, n, m, m_f, m_f_ext, nn, i4v_src, i4v_tgt, jg0, jg1, jgi, ig, nlev
    integer(I4B) :: nos, n_jg
    character(len=MXSLEN) :: f ! debug
! ------------------------------------------------------------------------------
    !
    ...
    allocate(xi4_int_mv(nc_f,nr_f)); xi4_int_mv = 0
    !
    ...
    !
    if (.false.) then
      f = 'c:\data\projects\project_Hegewarren\model\quad2d_fortran\_debug\xi4'
      call writeflt(f, xi4, nc_f, nr_f, bbx_f%xll, bbx_f%yll, bbx_f%cs, I4ZERO)
      stop
    end if
    ...
    if (renumber) then
      ...
      do ir = 1, nr_f
        do ic = 1, nc_f
          ...
          if (xi4_int_mv(ic,ir) == 1) then
            xi4(ic,ir) = -abs(xi4(ic,ir))
          end if
        end do
      end do
    end if
    ...
    if (allocated(xi4_int_mv)) deallocate(xi4_int_mv)
    !
    return
  end subroutine tQuad_get_grid

  subroutine tQuad_grid_gen(this, nl_act, lay_out, lskip, disu, lggok)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    !
    logical, parameter :: debug = .true.
    character(len=MXSLEN), parameter :: debug_d = 'c:\data\projects\project_Hegewarren\model\quad2d_fortran\_debug\'
    ...
    !
    character(len=MXSLEN) :: fp, f_binpos, clm
    !
    real(r4B),    dimension(:,:,:), allocatable :: zp
...
! ------------------------------------------------------------------------------
    ...
    lay_mod => this%lay_mods%get_lm_ptr(clm)
    ...
    if (lay_mod%nlay_ref > 0) then
      call lay_mod%read_extent_ref(bbx_read, zp, mv, lay_act, nl_act, mask)
    else
      call lay_mod%read_extent(bbx_read, zp, mv, lay_act, nl_act, mask)
    end if
    !
...
!    if (cs_gid <= cs_max_tgt) then ! changed 6-6-23
    ...
    do il = 1, nl
      ig = igrid_tgt(il) 
      call this%get_grid(ig, .true., xg, ng, bbx, n_offset=ncell) ! get quadtree grid at the finest level
      ncell = ncell + ng
      ...
      x(:,:,il) = xgf
      if ((il == 1).and.(.false.)) then
        fp = trim(debug_d)//ta([this%gid])//'_xg_l1'
        call writeflt(fp, xg, size(xg,1), size(xg,2), bbx%xll, bbx%yll, bbx%cs, 0)
        fp = trim(debug_d)//ta([this%gid])//'_xgf_l1'
        call writeflt(fp, xgf, size(xgf,1), size(xgf,2), bbx%xll, bbx%yll, bbx%cs, 0)
      end if
    end do
    !
    ! determine the bounding boxes
    if ((maxval(abs(x)) /= ncell)) then
      call logmsg('maxval(x)='//ta([maxval(abs(x))]))
    ...
    do i = 1, ncell
      ...
      rlev(i) = int(nint(log(real(n,R8B))/log(2.d0)),I4B) ! nint fix
    end do
    !
    ...
    do while(.true.)
      ...
      !
      if (n == 0) then
        call logmsg('Grid smoothing done after '//ta([n_try])//' tries...')
        exit
      else
...
! ------------------------------------------------------------------------------
    ...
    type(tMf6Wbd), pointer :: wbd => null()
    !
    character(len=MXSLEN) :: f_csv_dat_loc, id_pref_loc, f_binpos, s, fp, clm
...
! ------------------------------------------------------------------------------
    ...
    case(i_vrt_array_chd)
      call dmdat%mf6_get_data_vrt_array_array_chd(disu, bbx, i4a, r8a)
    case(i_csv)
...
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    use hdrModule, only: i_uscl_sumcdr_cinp
    ! -- dummy
    class(tDataModelData) :: this
    ...
    do i = 1, this%ndat
      ...
      if (dat%i_type == i_vrt) then
        call vrt_read_extent_mg(vrt=dat%vrt, bbx=bbx, csa=cs_read, &
          ...
          mfr8=dat%mult_fac, avr8=dat%add_val)
      end if
      if (dat%i_type == i_const) then
        call vrt_read_extent_mg(vrt=dat%vrt, bbx=bbx, csa=cs_read, &
          i_uscl=dat%i_uscl, i_dscl=dat%i_dscl, mg=mg, mvr4=mvr4, &
          mfr8=dat%mult_fac, avr8=dat%add_val, r8const=dat%r8const)
      end if
      ! resistance --> conductance
      if (dat%i_uscl == i_uscl_sumcdr_cinp) then
        do ig = 1, mg%ngrid
          g => mg%grid(ig)
          cs = g%bbx%cs
          mvr4 = cs*cs/g%mvr4
...
! ------------------------------------------------------------------------------
    ...
    !
    ! check that the vrta has exactly nlay layers
    if (vrta%n /= disu%nlay) then
      call errmsg('In file: '//ta([vrta%f]))
    end if
    !
  ...
  subroutine tDataModelData_mf6_get_data_vrt_array_array_chd(this, disu, bbx, i4a, r8a)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    logical :: lconst
    !
    integer(I4B) :: il, jl, ig, ir, ic, n, i, iact, nchd
    !
    real(R4B), dimension(:,:), allocatable :: r4x
    real(R4B) :: mvr4, r4const, r4v
    real(R8B) :: r8v
! ------------------------------------------------------------------------------
    ...
    do iact = 1, 2
      ...
      do jl = 1, disu%nlay_act
        ...
        do ig = 1, mg%ngrid
          if (mg%grid_count(ig) > 0) then
            ...
            do i = 1, g%n_int_mv
              ic = g%icir_int_mv(1,i); ir = g%icir_int_mv(2,i)
              n = g%xi4(ic,ir)
              if (lconst) then
                nchd = nchd + 1
                r8v = real(r4const, R8B)
                if (iact == 2) then
                  i4a(nchd) = abs(n); r8a(nchd) = r8v
                end if
              else
                if (r4x(ic,ir) /= mvr4) then
                  nchd = nchd + 1
                  r8v = real(r4x(ic,ir), R8B)
                  if (iact == 2) then
                    i4a(nchd) = abs(n); r8a(nchd) = r8v
                  end if
                end if
      ...
      !
      if ((iact == 1).and.(nchd > 0)) then
        allocate(i4a(nchd)); i4a = I4ZERO
        allocate(r8a(nchd)); r8a = R8ZERO
      end if
    end do !iact
    !
    ! clean up
    ...
    !
    return 
  end subroutine tDataModelData_mf6_get_data_vrt_array_array_chd
        ...
        do ip = 1, np
          if (point_in_bb(reshape([csv_x(ip),csv_y(ip)],[2,1]), bbx)) then
            ...
            if (valid_icr(ic, ir, nc, nr)) then
              do jl = 1, nl
                il = disu%lay_act(jl)
                n = abs(disu%grid_x_nod(ic, ir, jl))
                if ((n > 0).and.(csv_ilay(ip) == il)) then
                  mp = mp + 1
                  if (iact == 2) then
                    i4a(mp) = n
                    r8a(mp) = real(csv_v(ip), R8B)
                    r8a(mp) = r8a(mp) * dat%mult_fac ! apply factor
                    r8a(mp) = r8a(mp) + dat%add_val ! add value
                  end if
                end if
              end do
            end if
        ...
        do ip = 1, np
          if (point_in_bb(reshape([csv_x(ip),csv_y(ip)],[2,1]), bbx)) then
            ...
            if (valid_icr(ic, ir, nc, nr)) then
              lfound = .false.
              do il = 1, nl
                n = abs(disu%grid_x_nod(ic, ir, il))
                if (n > 0) then
                  if (.not.lfound) then
                    mp = mp + 1
            ...
            if (f > R4ZERO) then
              ...
              if (csv_v(ip) /= R4ZERO) then
                ...
                if (iact == 2) then
                  i4a(n) = nod2d(i,il)
                  ip = mapping(i)
                  r8a(n) = real(f, R8B) * real(csv_v(ip), R8B)
                  r8a(n) = r8a(n) * dat%mult_fac ! apply factor
                  r8a(n) = r8a(n) + dat%add_val ! add value
                end if
              end if
            end if
          end do
...
! ------------------------------------------------------------------------------
  ...
  subroutine vrt_read_extent_mg(vrt, bbx, csa, i_uscl, i_dscl, mg, &
    mvi1, mvi2, mvi4, mvi8, mvr4, mvr8, mfr8, avr8, r8const)
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    real(R8B),    intent(in), optional  :: r8const
    !
    ! -- local
    type(tBbx) :: bbx_read
    type(tGrid), pointer :: g   => null()
      ...
      if (g%i_data_type == i_r4) then
        if (.not.lconst) then
          call vrt%read_extent(xr4=g%xr4, mvr4=g%mvr4, bbx=g%bbx, &
            ...
            mfr8=mfr8_loc, avr8=avr8_loc)
        else
          g%xr4 = real(r8const*mfr8_loc + avr8_loc, r4B)
          g%mvr4 = g%xr4(1,1) + R4ONE
        end if
        mvr4 = g%mvr4
      else
        if (.not.lconst) then
          ...
          g%xr8 = r8const*mfr8_loc + avr8_loc
          g%mvr8 = g%xr8(1,1) + R8ONE
        end if
        mvr8 = g%mvr8
      end if
    end do
...
! ------------------------------------------------------------------------------
    if (cs_dst > cs_src) then
      f = log(cs_dst/cs_src)/log(2.0)
      nlev = nint(f) + 1
      nlev = -nlev
    else
      f = log(cs_src/cs_dst)/log(2.0)
      nlev = nint(f) + 1
    end if
    !
    return
